...
extends GwtFuturesCatchingSpecialization {
    ...
    public static <T> ImmutableList<ListenableFuture<T>> inCompletionOrder(Iterable<? extends ListenableFuture<? extends T>> futures) {
        ImmutableList<ListenableFuture<T>> copy = ImmutableList.copyOf(futures);
        ...
        for (ListenableFuture listenableFuture : copy) {
            listenableFuture.addListener(new Runnable((ImmutableList)delegates, listenableFuture){
                ...
                @Override
                public void run() {
                    for (int i = delegateIndex.get(); i < this.val$delegates.size(); ++i) {
                        if (!((SettableFuture)this.val$delegates.get(i)).setFuture(this.val$future)) continue;
